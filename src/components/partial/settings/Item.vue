<template>
  <div class="d-flex align-center w-100 h-100">
    ...
    <v-btn :data-cy="dataCy" v-else-if="item.type === 'button'" class="v-btn--is-elevated v-btn--has-bg v-size--default" @click="onClick">{{ $vuetify.lang.t(`$vuetify.settings.${category}.${name}.name`) }}</v-btn>
